...
;#	    print, next unless (@buf or $matched);
;#	    push(@readahead, $_);
;#	    next unless $code;
...
;#
sub jis_inout {
    $esc_jp = shift or $esc_jp;
    $esc_jp = "\e\$$esc_jp" if length($esc_jp) == 1;
    $esc_asc = shift or $esc_asc;
...
sub get_inout {
    ...
    $_[$[] =~ /$re_jp/o and ($esc_jp = $&);
    $_[$[] =~ /$re_asc/o and ($esc_asc = $&);
    ($esc_jp, $esc_asc);
}
...
sub convert {
    ...
    return (undef, undef) unless $icode = $icode or &getcode(*_);
...
sub h2z_sjis {
	...
	$1 or ($n++, $e2s{$h2z{$3}} or &e2s($h2z{$3}))/geo;
    $n;
}
...
sub init_z2h_euc {
    local($k, $_);
    s/[\241-\337]/\216$&/g and ($z2h_euc{$k} = $_) while ($k, $_) = each %z2h;
}
sub init_z2h_sjis {
    local($_, $v);
    /[\200-\377]/ and ($z2h_sjis{&e2s($_)} = $v) while ($_, $v) = each %z2h;
}
...
sub tr {
    ...
    if ($from ne $prev_from or $to ne $prev_to) {
